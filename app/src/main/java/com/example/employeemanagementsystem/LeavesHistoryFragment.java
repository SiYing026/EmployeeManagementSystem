...
public class LeavesHistoryFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_leaves_history, container, false);

        displayrecview = view.findViewById(R.id.recview);

        displayrecview.setLayoutManager(new LinearLayoutManager(getActivity()));
        FirebaseRecyclerOptions<Leaves> options = new FirebaseRecyclerOptions.Builder<Leaves>()
                .setQuery(FirebaseDatabase.getInstance().getReference().child("leaves")
                        .orderByChild("username")
                        .equalTo(identifier), Leaves.class)
                .build();

        displayAdapter = new LeavesAdapter(options);
